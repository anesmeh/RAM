...
class Ram:
    ...
    def _recuperer_element(self, element):
        ...
        if type(element) == int:
            ...
            try:
                ...
                raise ValueError(f"Impossible de récupérer l'élément à la position {position} de la liste.") from e
        else:
            raise TypeError(f"Le type de l'élément {element} n'est pas pris en charge.")


    def _lecture(self, instruction):
        """
            Analyse une instruction donnée et l'exécute en fonction de sa forme.

            Exemple:
            - lecture("ADD(5, I@r2, r5)") veut dire que l'on ajoute 5 et I@r2 dans r5.
            - lecture("JUMP(-3)") veut dire que l'on effectue un saut de -3.
        """
        commands = ("ADD", "SUB", "DIV", "MULT", "JUMP", "JE", "JL", "LOAD", "MOD")
    ...
    def step(self):
        self._lecture(self.instructs[self.pos])
    ...
    def _check_arth_error(self, arg):
        ...
        if len(arg) < 3:
            raise IndexError(f"La ligne {str(self.pos + 1)} contient une instruction invalide, il manque un/des argument(s): {self.instructs[self.pos]}.")
        # Exemple: ADD(a,b,c,d) avec a,b,c et d des ints ou registres
        if len(arg) > 3:
            raise IndexError(f"La ligne {str(self.pos + 1)} contient une instruction invalide, il y a trop d'arguments: {self.instructs[self.pos]}.")
        # Exemple: ADD(a,b,4) avec a et b des ints ou registres
        if type(arg[2]) == int:
            raise TypeError(f"{str(arg[0])} et {str(arg[1])}"+ "sont stockées dans l'int "+ str(arg[2])+ ".")
    ...
    def _fully_zero_and_check(self, arg):
        # Exemple: Pour r100 on remplit tout les registres d'avant à 0 afin d'éviter les erreurs.
        # Puis on check les erreurs
    ...
    def ADD(self, arg):
        pos = self._fully_zero_and_check(arg)
